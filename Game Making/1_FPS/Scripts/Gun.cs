...
public class Gun : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bullet_Scripta = bullet.gameObject.GetComponent<Bullet>();
    ...
    void Pucaj()
    {
        ...
        if(vrstaPucanja == 0) //Single shot
        {
            currentAmmo--; //ispucali smo metak, smanji za jedan
            ammo.text = currentAmmo + "/" + maxAmmo; //Novo stanje municije
            zvukPucanja.Play(); //zvuk ispucanog metka
            cloneBullet = Instantiate(bullet, bulletSpawnPoint.position, bulletSpawnPoint.rotation);
            cloneBullet.velocity = bullet_Scripta.speed * ray.direction;
        }

        if (vrstaPucanja == 1) //automatic shot
        {
            currentAmmo--; //ispucali smo metak, smanji za jedan
            ammo.text = currentAmmo + "/" + maxAmmo; //Novo stanje municije
            zvukPucanja.Play(); //zvuk ispucanog metka
        ...
        if(vrstaPucanja == 2) //burst fire - DZ!!!!!!
        {
            currentAmmo -= 3; //ispucali smo 3 metaka, smanji za tri
            ammo.text = currentAmmo + "/" + maxAmmo; //Novo stanje municije
            zvukPucanja.Play(); //zvuk ispucanog metka
