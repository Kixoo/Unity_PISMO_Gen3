    ...
    public void OnHighscoreDownload(Highscore.highscore[] highscoresList)
    {
        ...
        for (int i = 0; i < highscoresText.Length; i++)
        {
            ...
            if (highscoresList.Length > 1)
            {
                ...
                {
                    highscoresText[i].text += "Data does not exist";
                }
                else if (i < highscoresList.Length)
                {
        ...
        myHighscore.text = highscoreManager.userNick + " - " + PlayerPrefs.GetInt("Highscore") + " bodova";
